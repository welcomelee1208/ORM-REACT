...
router.get("/mychannels", async (req, res, next) => {
  ...
  try {
    ...
    C.channel_id,
    C.category_code,
    CASE WHEN C.category_code = 2 THEN C.channel_name ELSE (SELECT nick_name FROM channel_member WHERE channel_id=C.channel_id AND member_id != 1 LIMIT 1) END AS channel_name,
    CASE WHEN C.category_code = 2 THEN IFNULL(C.channel_img_path,'') ELSE (
    (SELECT profile_img_path FROM member WHERE member_id = (SELECT member_id FROM channel_member WHERE channel_id=C.channel_id AND member_id != 1 LIMIT 1))
    ...
    AND msg_type_code ='2' LIMIT 1) AS not_yet_cnt,
    L.message,
    L.msg_date
    FROM channel C 
